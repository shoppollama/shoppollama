...
INSTANCE_NAME="shoppollama-combined"
KEY_NAME="shoppollama-key"

echo "🚀 Starting EC2 deployment with Ollama on the same instance..."
...
sed -i.bak "s|__ECR_IMAGE_URI__|$ECR_REGISTRY/$ECR_REPOSITORY:$IMAGE_TAG|g" user-data-combined.sh
sed -i.bak "s|__ECR_REGISTRY__|$ECR_REGISTRY|g" user-data-combined.sh
...
# Create EC2 instance with larger size (t4g.large) and bigger storage
echo "🔧 Creating EC2 instance with Ollama support..."
INSTANCE_ID=$(aws ec2 run-instances \
  ...
  --user-data file://user-data-combined.sh \
