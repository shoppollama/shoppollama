defmodule ShoppollamaWeb.OAuthController do
  ...
  def custom(conn, %{
    ...
    if valid_shop_domain?(shop) do
      # Save custom app credentials to store
      store_params = %{
        shop_domain: shop,
        access_token: access_token,
      ...
      case create_or_update_store(store_params) do
        ...
        {:error, changeset} ->
          Logger.error("Failed to save custom app store: #{inspect(changeset.errors)}")

          conn
          |> put_flash(:error, "Failed to save custom app store information")
          |> redirect(to: "/auth/shopify")
      end
    else
      conn
      |> put_flash(:error, "Invalid shop domain")
